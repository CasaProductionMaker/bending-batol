...
(function() {
    ...
    if(players[playerId] != null) {
      ...
      Object.keys(players[playerId].electrons).forEach((electron) => {
        ...
        Object.keys(mobs).forEach((mob) => {
          ...
          if(mobStats[thisMob.type] != undefined && (Math.abs(thisMob.x - thisElectron.x) < 96 || Math.abs(thisMob.y - thisElectron.y) < 96) && distanceBetween(thisElectron.x, thisElectron.y, thisMob.x, thisMob.y) <= electronStats[thisElectron.type].Size + mobStats[thisMob.type].Size)
          {
      ...
      //Mobs
      let mobKeys = Object.keys(mobs);
      if(batch == 0)
      {
        mobKeys = mobKeys.slice(0, Math.floor(mobKeys.length/2));
      } else {
        mobKeys = mobKeys.slice(Math.floor(mobKeys.length/2));
      }
      batch = 1 - batch;
      mobKeys.forEach((mob) => {
        const thisMob = mobs[mob];
        //Move
        if(thisMob.type == "Screw" || thisMob.type == "LaserScrew" || thisMob.type == "Missile")
        {
          let TargetX = 0;
          let TargetY = 0;
          let shortestDist = mapSize * 3;
          if(thisMob.targetUser != undefined && thisMob.targetUser == 0)
          {
            Object.keys(players).forEach((key) => {
              ...
              if(distanceBetween(thisMob.x, thisMob.y, characterState.x, characterState.y) < shortestDist)
              {
                ...
                thisMob.targetUser = characterState.id;
                shortestDist = distanceBetween(thisMob.x, thisMob.y, characterState.x, characterState.y);
              }
            })
          } else {
            TargetX = players[thisMob.targetUser].x;
            TargetY = players[thisMob.targetUser].y;
          }
          ...
          if(thisMob.type == "Screw" || thisMob.type == "LaserScrew")
          {
            //if(shortestDist < mapSize / 2)
            //{
              //thisMob.xV *= 1.18;
              //thisMob.yV *= 1.18;
            //} else {
            ...
            //}
            if(TargetX > thisMob.x) thisMob.xV = mobStats[thisMob.type].Speed;
            if(TargetX < thisMob.x) thisMob.xV = -mobStats[thisMob.type].Speed;
            if(TargetY > thisMob.y) thisMob.yV = mobStats[thisMob.type].Speed;
            if(TargetY < thisMob.y) thisMob.yV = -mobStats[thisMob.type].Speed;
            thisMob.direction += 20;
          }
          if(thisMob.type == "Missile")
          {
            thisMob.xV = Math.sin(targetDir * Math.PI / 180.0) * mobStats[thisMob.type].Speed;
            thisMob.yV = Math.cos(targetDir * Math.PI / 180.0) * mobStats[thisMob.type].Speed;
        ...
        Object.keys(mobs).forEach((innerMob) => {
          ...
          if((Math.abs(thisMob.x - mobColl.x) < 128 || Math.abs(thisMob.y - mobColl.y) < 128) && distanceBetween(thisMob.x, thisMob.y, mobColl.x, mobColl.y) < mobStats[thisMob.type].Size + mobStats[mobColl.type].Size && thisMob.id != mobColl.id)
          {
    ...
    if(players[playerId] != null) {
      ...
      if(gameHost == localStorage.getItem("AtomixUser"))
      {
        if(mobsSpawned < Wave * 2)
        {
          ...
          while((Math.abs(players[playerId].x - nMX) < 128 || Math.abs(players[playerId].y - nMY) < 128) && distanceBetween(players[playerId].x, players[playerId].y, nMX, nMY) < 128)
          {
  ...
  function handleMovement(xChange=0, yChange=0) {
    ...
    if (!players[playerId].isDead) {
      ...
      Object.keys(mobs).forEach((mob) => {
        const thisMob = mobs[mob];
        if(mobStats[thisMob.type] != undefined && (Math.abs(players[playerId].x - thisMob.x) < 15.0 + mobStats[thisMob.type].Size || Math.abs(players[playerId].y - thisMob.y) < 15.0 + mobStats[thisMob.type].Size) && distanceBetween(players[playerId].x, players[playerId].y, thisMob.x, thisMob.y) <= 13.0 + mobStats[thisMob.type].Size)
        {
  ...
  function initGame() {
    ...
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron1"), "SharpElectron", 0, 0, "electron1");
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron2"), "TankElectron", 1, 0, "electron2");
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron3"), "SharpElectron", 2, 0, "electron3");
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron4"), "TankElectron", 3, 0, "electron4");
    ...
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron7"), "SharpElectron", 0, 1, "electron7");
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron8"), "SpedElectron", 1, 1, "electron8");
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron9"), "SharpElectron", 2, 1, "electron9");
    ...
    addElectron(firebase.database().ref("games/" + gameCode + "/players/" + playerId + "/electrons/electron12"), "SpedElectron", 5, 1, "electron12");
